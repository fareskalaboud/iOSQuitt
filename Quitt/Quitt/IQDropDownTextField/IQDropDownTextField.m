...
@implementation IQDropDownTextField {
    ...
    UIDatePicker *dateAndTimePicker;
    NSDateFormatter *dropDownDateFormatter;
    NSDateFormatter *dropDownTimeFormatter;
    NSDateFormatter *dropDownDateAndTimeFormatter;
}
...
- (void)initialize {
    ...
    dropDownDateAndTimeFormatter = [[NSDateFormatter alloc] init];
    [dropDownDateAndTimeFormatter setTimeStyle:NSDateFormatterShortStyle];
    [dropDownDateAndTimeFormatter setDateStyle:NSDateFormatterShortStyle];
    ...
    dateAndTimePicker = [[UIDatePicker alloc] init];
    [dateAndTimePicker setAutoresizingMask:(UIViewAutoresizingFlexibleWidth|UIViewAutoresizingFlexibleHeight)];
    [dateAndTimePicker setDatePickerMode:UIDatePickerModeDateAndTime];
    [dateAndTimePicker addTarget:self action:@selector(dateAndTimeChanged:) forControlEvents:UIControlEventValueChanged];
...
- (void)setDropDownMode:(IQDropDownMode)dropDownMode {
    ...
    switch (_dropDownMode)
    {
        ...
        case IQDropDownModeTimePicker:
            ...
            break;
        case IQDropDownModeDateAndTimePicker:
            self.inputView = dateAndTimePicker;
            break;
        default:
            break;
    }
...
- (void)dateAndTimeChanged:(UIDatePicker *)datPicker {
    [self setSelectedItem:[dropDownDateAndTimeFormatter stringFromDate:datPicker.date]];
}
...
- (void)setSelectedItem:(NSString *)selectedItem {
    switch (_dropDownMode)
    {
        ...
        case IQDropDownModeDateAndTimePicker:
        {
            NSDate *date = [dropDownDateAndTimeFormatter dateFromString:selectedItem];
            if (date) {
                _selectedItem = selectedItem;
                [self setText:selectedItem];
                [datePicker setDate:date animated:YES];
            } else {
                NSLog(@"Invalid time/date or time/date format:%@", selectedItem);
            }
            break;
        }
    }
